...
# Step 2: Reshape the data to wide format for Friedman test
UMSARS2 <- UMSARS2 %>%    filter(num_patient != 1 & num_patient!= 2 & num_patient !=3) %>%
...
QOL_AMS %>% ungroup() %>% group_by(num_visite) %>%  
    filter(num_patient != 1 & num_patient!= 2 & num_patient !=3) %>%
  summarise(mean=mean(Motor ), se=sd(Motor ),
            median=median(Motor ),
...
QOL_AMS %>% ungroup() %>% group_by(num_visite) %>%  
      filter(num_patient != 1 & num_patient!= 2 & num_patient !=3) %>%
  summarise(mean=mean(Non_Motor), se=sd(Non_Motor),
            median=median(Non_Motor),
...
QOL_AMS %>% ungroup() %>% group_by(num_visite) %>% 
      filter(num_patient != 1 & num_patient!= 2 & num_patient !=3) %>%
  summarise(mean=mean(Emotional), se=sd(Emotional),
            median=median(Emotional),
            q25=quantile(Emotional, 0.25),
...
QOL_AMS %>%
      filter(num_patient != 1 & num_patient!= 2 & num_patient !=3) %>%
  ggplot(aes(num_visite , Motor, colour=num_visite , fill=num_visite )) +
  geom_boxplot(alpha=0.7, notch = TRUE, width = 0.5, outlier.shape = NA) +
  geom_jitter(size=3,shape=1,stroke=2.5, width=0.3 ) +
  theme(axis.text.y = element_blank(),
        axis.ticks.y = element_blank(),
...
QOL_AMS %>%
      filter(num_patient != 1 & num_patient!= 2 & num_patient !=3) %>%
  ggplot(aes(num_visite , Non_Motor, colour=num_visite , fill=num_visite )) +
  geom_boxplot(alpha=0.7, notch = TRUE, width = 0.5, outlier.shape = NA) +
  geom_jitter(size=3,shape=1,stroke=2.5, width=0.3 ) +
  theme(axis.text.y = element_blank(),
        axis.ticks.y = element_blank(),
...
QOL_AMS %>%
      filter(num_patient != 1 & num_patient!= 2 & num_patient !=3) %>%
  ggplot(aes(num_visite , Emotional, colour=num_visite , fill=num_visite )) +
  geom_boxplot(alpha=0.7, notch = TRUE, width = 0.5, outlier.shape = NA) +
  geom_jitter(size=3,shape=1,stroke=2.5, width=0.3 ) +
  theme(axis.text.y = element_blank(),
        axis.ticks.y = element_blank(),
...
# Step 2: Reshape the data to wide format for Friedman test
QOL_AMS_Motor <- QOL_AMS %>% select(num_patient , num_visite, Motor) %>%
      filter(num_patient != 1 & num_patient!= 2 & num_patient !=3) %>%
  tidyr::pivot_wider(names_from = num_visite, values_from = Motor )

QOL_AMS_Non_Motor <- QOL_AMS %>% select(num_patient , num_visite, Non_Motor ) %>%
      filter(num_patient != 1 & num_patient!= 2 & num_patient !=3) %>%
  tidyr::pivot_wider(names_from = num_visite, values_from = Non_Motor  )

QOL_AMS_Emotional <- QOL_AMS %>% select(num_patient , num_visite, Emotional ) %>%
      filter(num_patient != 1 & num_patient!= 2 & num_patient !=3) %>%
  tidyr::pivot_wider(names_from = num_visite, values_from = Emotional  )



# Step 3: Friedman Test
friedman_result <- friedman.test(as.matrix(QOL_AMS_Motor[, c("V0", "V1", "V2")]))
print(friedman_result)

# 	Friedman rank sum test
# 
# data:  as.matrix(QOL_AMS_Motor[, c("V0", "V1", "V2")])
# Friedman chi-squared = 11.32, df = 2, p-value = 0.003483

friedman_result <- friedman.test(as.matrix(QOL_AMS_Non_Motor[, c("V0", "V1", "V2")]))
print(friedman_result)


# 	Friedman rank sum test
# 
# data:  as.matrix(QOL_AMS_Non_Motor[, c("V0", "V1", "V2")])
# Friedman chi-squared = 2.3922, df = 2, p-value = 0.3024

friedman_result <- friedman.test(as.matrix(QOL_AMS_Emotional[, c("V0", "V1", "V2")]))
print(friedman_result)
...
# 	Friedman rank sum test
# 
# data:  as.matrix(QOL_AMS_Emotional[, c("V0", "V1", "V2")])
# Friedman chi-squared = 2.52, df = 2, p-value = 0.2837



# Step 4: Post-hoc test (Pairwise comparisons using Nemenyi test)
data_matrix <- as.matrix(QOL_AMS_Motor[, c("V0", "V1", "V2")])
rownames(data_matrix) <- QOL_AMS_Motor$num_patient  # Assign patient IDs as row names
...
data_matrix <- as.matrix(QOL_AMS_Non_Motor[, c("V0", "V1", "V2")])
rownames(data_matrix) <- QOL_AMS_Non_Motor$num_patient  # Assign patient IDs as row names
...
data_matrix <- as.matrix(QOL_AMS_Emotional[, c("V0", "V1", "V2")])
rownames(data_matrix) <- QOL_AMS_Emotional$num_patient  # Assign patient IDs as row names
...
pats_to_match <- pats_to_match %>% filter(num_patient != 1 & num_patient!= 2 & num_patient !=3) 
...
BaselineDem <- BaselineDem %>% select(NUM, DATECONSULT, SEXE, AGE_VISITE0, UMSARS1_TOT , UMSARS2_TOT, UMSARS_tot, SCHRAG_TOT_v2, SCHRAG_MOTOR_v2 , SCHRAG_NONMOTOR_v2 , SCHRAG_EMOTIONAL_v2 ,diff)

names(BaselineDem) <- c("num_patient", "DATECONSULT",  "sexe", "age_inclusion", "UMSARS1_TOT", "UMSARS2_TOT", "UMSARS_tot" , "QOL_AMS_total", "SCHRAG_MOTOR_v2", "SCHRAG_NONMOTOR_v2", "SCHRAG_EMOTIONAL_v2", "diff")
...
UMSARS <- UMSARS1 %>% inner_join(UMSARS2) %>% mutate(UMSARS_tot=UMSARS1_total+UMSARS2_total) 

UMSARS <- UMSARS %>% filter(num_patient!=1&num_patient!=2&num_patient!=8) 


UMSARS %>%  filter(num_visite=="V0"|num_visite=="V2") %>%
  select(num_patient, num_visite, UMSARS_tot) %>%
...
UMSARS %>%
  ...
  filter(num_visite=="V0"|num_visite=="V2") %>%
      select(num_patient, num_visite, UMSARS_tot) %>%
  spread(key=num_visite, value=UMSARS_tot) %>%
  drop_na() %>% mutate(delta=V2-V0) %>% select(-c(V0, V2)) %>% 
  mutate(group="TEP") %>% ungroup() %>% select(-num_patient)  %>%
...
UMSARS %>% filter(num_visite=="V0"|num_visite=="V2") %>%
  select(num_patient, num_visite, UMSARS1_total ) %>%
  spread(key=num_visite, value=UMSARS1_total ) %>%
  drop_na() %>% mutate(delta=V2-V0) %>% select(-c(V0, V2)) %>% 
  mutate(group="TEP") %>% ungroup() %>% select(-num_patient)  %>%
   summarise(mean=mean(delta), se=sd(delta),
            median=median(delta),
...
filtered %>% select(matched_to, num_patient, UMSARS1_TOT,visite  ) %>%
  spread(key=visite, value=UMSARS1_TOT) %>% drop_na() %>%
  mutate(delta=`2`-`1`) %>% select(-c(`1`, `2`)) %>% ungroup() %>%
  group_by(matched_to) %>% sample_n(3, replace=T) %>% ungroup() %>%
  mutate(group="Control") %>% select(-c(matched_to, num_patient)) %>%
   summarise(mean=mean(delta), se=sd(delta),
...
control_table <- filtered %>% select(matched_to, num_patient, UMSARS1_TOT,visite  ) %>%
  spread(key=visite, value=UMSARS1_TOT) %>% drop_na() %>%
  mutate(delta=`2`-`1`) %>% select(-c(`1`, `2`)) %>% ungroup() %>%
  group_by(matched_to) %>% sample_n(3, replace=T) %>% ungroup() %>%
  mutate(group="Control") 
...
UMSARS %>% filter(num_visite=="V0"|num_visite=="V2") %>%
  select(num_patient, num_visite, UMSARS2_total ) %>%
  spread(key=num_visite, value=UMSARS2_total ) %>%
  drop_na() %>% mutate(delta=V2-V0) %>% select(-c(V0, V2)) %>% 
  mutate(group="TEP") %>% ungroup() %>% select(-num_patient)  %>%
   summarise(mean=mean(delta), se=sd(delta),
            median=median(delta),
...
#    mean    se median   q25   q75
# 1  7.17  9.50    4.5   0.5  11.2

filtered %>% select(matched_to, num_patient, UMSARS2_TOT,visite  ) %>%
  spread(key=visite, value=UMSARS2_TOT) %>% drop_na() %>%
  mutate(delta=`2`-`1`) %>% select(-c(`1`, `2`)) %>% ungroup() %>%
  group_by(matched_to) %>% sample_n(3, replace=T) %>% ungroup() %>%
  mutate(group="Control") %>% select(-c(matched_to, num_patient)) %>%
   summarise(mean=mean(delta), se=sd(delta),
...
control_table <- filtered %>% select(matched_to, num_patient, UMSARS2_TOT,visite  ) %>%
  spread(key=visite, value=UMSARS2_TOT) %>% drop_na() %>%
  mutate(delta=`2`-`1`) %>% select(-c(`1`, `2`)) %>% ungroup() %>%
  group_by(matched_to) %>% sample_n(3, replace=T) %>% ungroup() %>%
  mutate(group="Control") 
...
QOL_AMS %>% filter(num_visite=="V0"|num_visite=="V2") %>%
  select(num_patient, num_visite, QOL_AMS_total ) %>%
...
QOL_AMS %>% filter(num_visite=="V0"|num_visite=="V2") %>%
      filter(num_patient!=1&num_patient!=2&num_patient!=8) %>%
  select(num_patient, num_visite, Emotional) %>%
  spread(key=num_visite, value=Emotional ) %>% 
  drop_na() %>% mutate(delta=V2-V0) %>% select(-c(V0, V2)) %>% 
  mutate(group="TEP") %>% ungroup() %>% select(-num_patient)  %>%
   summarise(mean=mean(delta), se=sd(delta),
            median=median(delta),
...
filtered %>% select(matched_to, num_patient, SCHRAG_EMOTIONAL_v2   ,visite  ) %>%
  spread(key=visite, value=SCHRAG_EMOTIONAL_v2   ) %>% drop_na() %>%
  mutate(delta=`2`-`1`) %>% select(-c(`1`, `2`)) %>% ungroup() %>%
  group_by(matched_to) %>% sample_n(3, replace=T) %>% ungroup() %>%
  mutate(group="Control") %>% select(-c(matched_to, num_patient)) %>%
   summarise(mean=mean(delta), se=sd(delta),
...
control_table <- filtered %>% select(matched_to, num_patient, SCHRAG_EMOTIONAL_v2,visite  ) %>%
  spread(key=visite, value=SCHRAG_EMOTIONAL_v2) %>% drop_na() %>%
  mutate(delta=`2`-`1`) %>% select(-c(`1`, `2`)) %>% ungroup() %>%
  group_by(matched_to) %>% sample_n(3, replace=T) %>% ungroup() %>%
  mutate(group="Control") 
...
QOL_AMS %>% filter(num_visite=="V0"|num_visite=="V2") %>% 
  ...
  scale_fill_manual(values=c("#bf4438", "#333652")) +
  xlab("\n Group Intervention") +
  ylab("Delta MSA QoL Emotional \n [12 months] \n") +
   geom_hline(yintercept=0)

# ------------------
# Extra disease duration and caregiver info yellow cols ----


Histoire_AMS <- read_excel(path = "afewanalysisformsaetp/BDD_ETP_AMS_7122023 MF.xlsx", sheet="Histoire_AMS")



names(Histoire_AMS)

Histoire_AMS %>% select(`Disease duration at baseline (months), considering motor symptoms onset`) %>%
  rename("var"="Disease duration at baseline (months), considering motor symptoms onset") %>%
  summarise(mean=mean(var), sd=sd(var), median=median(var), q1=quantile(var, 0.25), q3=quantile(var, 0.75))


Histoire_AMS %>% select(`Caregiver age`) %>%
  rename("var"="Caregiver age") %>%
  summarise(mean=mean(var), sd=sd(var), median=median(var), q1=quantile(var, 0.25), q3=quantile(var, 0.75))
...
Satisfaction_programme <- read_excel(path = "data/BDD_ETP_AMS_7122023.xlsx", sheet="Satisfaction_programme")


Satisfaction_programme <- Satisfaction_programme %>% 
  select(num_patient, num_visite, satisfaction_pg_item1:satisfaction_pg_item9)

Satisfaction_programme <- Satisfaction_programme %>% 
  filter(num_patient!=1&num_patient!=2&num_patient!=8)

Satisfaction_programme <- Satisfaction_programme %>% drop_na()

names(Satisfaction_programme) <- c("num_patient", "num_visite",
                                   "item1",  "item2",  "item3",  "item4", 
                                   "item5",  "item6",  "item7",  "item8",  "item9" )


df <- data.frame(Satisfaction_programme %>% gather(item, score, item1:item9) %>%
  group_by(num_visite, item, score) %>% count() %>%
    ungroup() %>% group_by(num_visite, item) %>% mutate(tot=sum(n)) %>% mutate(perc=n/tot))


df <- df %>% mutate(item=ifelse(item=="item1", "The training is tailored to your needs",
                          ifelse(item=="item2", "The training objectives are met",
                                 ifelse(item=="item3", "The explanations are clear",
                                        ifelse(item=="item4", "What you have learned is applicable in daily life",
                                               ifelse(item=="item5", "What you have learned has been useful to you",
                                                      ifelse(item=="item6", "The duration of the sessions seemed to you",
                                                             ifelse(item=="item7", "The training met your expectations",
                                                                    ifelse(item=="item8", "The program format is appropriate",
                                                                           ifelse(item=="item9", "The digital tool is easy to use", NA))))))))))


df <- df %>% mutate(score=ifelse(score==1, "1- Slightly satisfied",
                                 ifelse(score==2, "2- Satisfied", "3- Very satisfied")))
  

df$score <- factor(df$score, levels = c("1- Slightly satisfied", "2- Satisfied", "3- Very satisfied"))

# Plot for V1
df %>%
  filter(num_visite == "V1") %>%
  ggplot(aes(x = item, y = perc, fill = score)) +
  ...
  labs(
    title = "Score Distribution per Question - Visit V1",
    x = "Question \n",
    y = "\n Percentage",
    fill = "Score"
  ) +
...
df %>%
  ...
  scale_y_continuous(labels = scales::percent) +
  labs(
    title = "Score Distribution per Question - Visit V2",
    x = "Question \n",
    y = "\n Percentage",
    fill = "Score"
  ) +
