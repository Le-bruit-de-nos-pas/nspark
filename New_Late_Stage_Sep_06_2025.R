...
sum(med$ASC)/1109
sum(med$F)/1109
sum(med$Antipsychotique)/1109
sum(med$Antidepresseur)/1109


fwrite(med, "med_late_stage.csv")
...
data.frame(long_med %>% drop_na() %>% group_by(VISIT , symptom, Group) %>%
...
to_print <- long_med %>% group_by(Group, symptom) %>%
   summarise(mean=mean(diff, na.rm=T), sd=sd(diff, na.rm=T), n=n()) %>%
  mutate(mean=round(mean,2), sd =round(sd, 2)) %>%
  mutate(mean=paste0(mean, paste0(" ± ", sd))) %>% select(-sd) 

fwrite(to_print, "late_stage_deltas_v1-v2_meansd_number.csv")
...
fwrite(final_results, "final_results_Late_Stage_PD_Sep_10.csv")
