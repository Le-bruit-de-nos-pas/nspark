...
ecmp <- ecmp %>% 
  mutate(
    ...
    ecmp_icd_binary = if_else(rowSums(select(., ECMP12, ECMP17, ECMP18, ECMP19) >= 1, na.rm = TRUE) >= 1,1, 0),
...
# 1 Amantadine    98    32    32.7
# 2 Placebo      106    35    33.0
...
length(unique(incident_icd_data$SUBJID)) # 137
...
length(unique(first_icd_per_patient$SUBJID)) # 49

last_cencor_per_patient <- incident_icd_data %>% anti_join(first_icd_per_patient %>% select(SUBJID)) %>%
  select(SUBJID, VISIT) %>% group_by(SUBJID) %>%
  filter(VISIT==max(VISIT)) %>% distinct()

length(unique(last_cencor_per_patient$SUBJID)) # 88

first_icd_per_patient %>% bind_rows(last_cencor_per_patient) # 137
...
# TREATMENT=Amantadine 66       24     22.9    0.0506     0.113
# TREATMENT=Placebo    71       25     26.1    0.0445     0.113
# 
...
# 
#   n= 137, number of events= 49 
# 
#                     coef exp(coef) se(coef)      z Pr(>|z|)
# TREATMENTPlacebo -0.1010    0.9039   0.2859 -0.353    0.724
# 
#                  exp(coef) exp(-coef) lower .95 upper .95
# TREATMENTPlacebo    0.9039      1.106    0.5162     1.583
# 
# Concordance= 0.518  (se = 0.039 )
...
plot <- ggplot(plot_data, aes(x = factor(VISIT), y = perc, fill = ecmp_icd_severity)) +
  ...
  facet_wrap(~TREATMENT, ncol = 2) +
    ...
    scale_fill_manual(values=c(  "#c1cfd1", "#6F9196", "#52767D", "#3E506B",  "#584b6e", "#44365C" )) +
  theme_minimal() +
   theme(text = element_text(face = "bold"),
    axis.text = element_text(size = 10),
    axis.title = element_text(size = 10),
