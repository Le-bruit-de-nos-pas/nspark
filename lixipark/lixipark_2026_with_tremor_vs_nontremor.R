...
temp_long <- temp %>%
  select(
    ...
    updrs3_tremor_visit2 = `updrs3_tremor_total_Visit 2 - Baseline`,
    updrs3_tremor_visit6 = `updrs3_tremor_total_Visit 6 - Follow-up`,
    updrs3_tremor_visit8 = `updrs3_tremor_total_Visit 8 - Follow-up`,

    updrs3_nontremor_visit2 = `updrs3_nontremor_total_Visit 2 - Baseline`,
    updrs3_nontremor_visit6 = `updrs3_nontremor_total_Visit 6 - Follow-up`,
    updrs3_nontremor_visit8 = `updrs3_nontremor_total_Visit 8 - Follow-up`
  ) %>%
  pivot_longer(
  ...
  mutate(
    scale = case_when(
      str_detect(variable, "updrs3_nontremor") ~ "MDS-UPDRS III Non-Tremor",
      str_detect(variable, "updrs3_tremor") ~ "MDS-UPDRS III Tremor",
      str_detect(variable, "non_tremor") ~ "Non-Tremor Scale",
      str_detect(variable, "tremor_") ~ "Tremor Scale",
      str_detect(variable, "eleven") ~ "11-Item Scale",
      str_detect(variable, "mdsIII") ~ "MDS-UPDRS Part III",
      TRUE ~ NA_character_
    ),
...
primary_scales <- c("MDS-UPDRS Part III", "MDS-UPDRS III Non-Tremor", "MDS-UPDRS III Tremor", "Non-Tremor Scale",  "Tremor Scale", "11-Item Scale")
...
plot_primary <- temp_long %>%
  ...
  facet_wrap(~scale, scales = "free_y", nrow = 6) +
...
ggsave(file="../out/4_scales_3_time_points_exact_score.svg", plot=plot_primary, width=5, height=12)
...
summary_comparisons <- temp_long %>%
  ...
  mutate(
    ...
    scale = factor(scale, levels = c("Tremor Scale", "Non-Tremor Scale", "11-Item Scale",
                                     "MDS-UPDRS Part III", "MDS-UPDRS III Tremor", "MDS-UPDRS III Non-Tremor"))
  ) %>%
  arrange(scale, visit)
...
#  2 Tremor Scale             Month 6  4.8 ± 3.9 | 4 [2-7]         3 ± 2.8 | 2 [1-5]             0.005            
...
#  5 Non-Tremor Scale         Month 6  18.6 ± 10.8 | 17 [11-25]    17.9 ± 10.9 | 16.5 [9.8-24.2] 0.59             
...
#  8 11-Item Scale            Month 6  4.9 ± 4.6 | 3 [1-8]         4.8 ± 4.3 | 4 [1.8-6]         0.96             
...
# 11 MDS-UPDRS Part III       Month 6  17.5 ± 9.1 | 18 [11-22]     15.4 ± 8.4 | 14.5 [9-20.2]    0.12             
# 12 MDS-UPDRS Part III       Month 12 18.5 ± 9.9 | 18 [11.8-23.2] 14.9 ± 7.4 | 14 [9-20]        0.02             
# 13 MDS-UPDRS III Tremor     Baseline 3.1 ± 2.9 | 2 [1-5]         2.3 ± 2.4 | 2 [0-4]           0.07             
# 14 MDS-UPDRS III Tremor     Month 6  3.7 ± 3.4 | 3 [1-6]         2.1 ± 2.3 | 1 [0-3.2]         0.003            
# 15 MDS-UPDRS III Tremor     Month 12 3.8 ± 3.6 | 3 [0-6]         2.6 ± 2.8 | 2 [0-4]           0.04             
# 16 MDS-UPDRS III Non-Tremor Baseline 12.7 ± 6.8 | 12 [7-18]      12.6 ± 6.6 | 11 [8-16.8]      0.88             
# 17 MDS-UPDRS III Non-Tremor Month 6  13.8 ± 7.7 | 13 [9-18]      13.2 ± 8.1 | 12 [6-17.2]      0.5              
# 18 MDS-UPDRS III Non-Tremor Month 12 14.7 ± 8.5 | 14 [8.8-19]    12.3 ± 6.8 | 13 [7-15]        0.1
...
ggsave(file="../out/4_scales_3_time_points_means.svg", plot=plot_primary_bars, width=5, height=12)
...
sc_wide <- sc_wide %>%
  ...
  select(USUBJID, tremor_chg_6, tremor_chg_12, non_tremor_chg_6, non_tremor_chg_12, 
         eleven_chg_6, eleven_chg_12, mdsIII_chg_6, mdsIII_chg_12, mdsIII_tremor_chg_6, mdsIII_tremor_chg_12,mdsIII_nontremor_chg_6,mdsIII_nontremor_chg_12 )
...
normality_tests <- sc_wide %>%
  select(tremor_chg_6, tremor_chg_12, 
         ...
         mdsIII_tremor_chg_6, mdsIII_tremor_chg_12,mdsIII_nontremor_chg_6,mdsIII_nontremor_chg_12 ) %>%
...
normality_by_group <- sc_wide %>%
  select(RAN_GRP_LIB, tremor_chg_6:tremor_chg_12, 
         ...
         eleven_chg_6:eleven_chg_12,
         mdsIII_chg_6:mdsIII_chg_12,
         mdsIII_tremor_chg_6:mdsIII_tremor_chg_12 ,
         mdsIII_nontremor_chg_6:mdsIII_nontremor_chg_12 ) %>%
...
delta_long <- sc_wide %>%
 ...
 mutate(
    scale = case_when(
      str_detect(variable, "mdsIII_nontremor") ~ "MDS-UPDRS III Non-Tremor",
      str_detect(variable, "mdsIII_tremor") ~ "MDS-UPDRS III Tremor",
      str_detect(variable, "non_tremor_chg") ~ "Non-Tremor Scale",
      str_detect(variable, "tremor_chg") ~ "Tremor Scale",
      str_detect(variable, "eleven_chg") ~ "11-Item Scale",
      str_detect(variable, "mdsIII_chg") ~ "MDS-UPDRS Part III",
      TRUE ~ NA_character_
    ),
...
desired_scale_order <- c("MDS-UPDRS Part III", "MDS-UPDRS III Non-Tremor", "MDS-UPDRS III Tremor", "Non-Tremor Scale", "Tremor Scale", "11-Item Scale")
...
ggsave(file="../out/deltas_box.svg", plot=plot_delta_box, width=3, height=12)
...
ggsave(file="../out/deltas_bar.svg", plot=plot_delta_bars, width=3, height=10)
...
#  1 MDS-UPDRS Part III       1 - Placebo           35    40       4.66     1.62          3        0         6  0.0828 0.08        ""      
#  2 MDS-UPDRS Part III       2 - Lixisenatide      43    34      -0.56     0.62         -1       -5         2  0.456  0.46        ""      
#  3 MDS-UPDRS III Non-Tremor 1 - Placebo           35    40       3.4      1.35          2       -1         5  0.183  0.18        ""      
#  4 MDS-UPDRS III Non-Tremor 2 - Lixisenatide      43    34      -0.72     0.21         -1       -4         2  0.494  0.49        ""      
#  5 MDS-UPDRS III Tremor     1 - Placebo           35    40       1.26     0.28          1        0         2  0.0778 0.08        ""      
#  6 MDS-UPDRS III Tremor     2 - Lixisenatide      43    34       0.16     0.41          0       -1         1  0.626  0.63        ""      
#  7 Non-Tremor Scale         1 - Placebo           35    40       4.74     2.55          2       -1         5  0.223  0.22        ""      
#  8 Non-Tremor Scale         2 - Lixisenatide      43    34       0.67     1.5           0       -4         3  0.967  0.97        ""      
#  9 Tremor Scale             1 - Placebo           35    40       1.54     0.28          1        0         2  0.0384 0.04        "*"     
# 10 Tremor Scale             2 - Lixisenatide      43    34       0.33     0.53          0       -1         1  0.770  0.77        ""      
# 11 11-Item Scale            1 - Placebo           35    40       1.66     1.12          0       -1         2  0.704  0.7         ""      
# 12 11-Item Scale            2 - Lixisenatide      43    34       1.42     1.15          0       -1         2  0.657  0.66        ""  
...
smd_plot_data <- smd_results %>%
  mutate(
    ...
    scale = factor(scale, levels = c("MDS-UPDRS Part III", "MDS-UPDRS III Non-Tremor" , "MDS-UPDRS III Tremor", "Non-Tremor Scale", 
...
ggsave(file = "../out/smd_forest_plot.svg", plot = smd_plot, width = 10, height = 5)
...
# 1 MDS-UPDRS Part III       Month 6     140       1.2        5.85 0.205  0.085        0.038        0.373 small         0.205 [0.038-0.373] small      
#  2 MDS-UPDRS Part III       Month 12    152       1.48       7.06 0.21   0.082        0.049        0.37  small         0.21 [0.049-0.37] small        
#  3 MDS-UPDRS III Non-Tremor Month 6     140       0.986      5.27 0.187  0.085        0.02         0.354 negligible    0.187 [0.02-0.354] negligible  
#  4 MDS-UPDRS III Non-Tremor Month 12    152       0.98       5.96 0.165  0.082        0.005        0.325 negligible    0.165 [0.005-0.325] negligible 
#  5 MDS-UPDRS III Tremor     Month 6     140       0.214      1.93 0.111  0.085       -0.055        0.277 negligible    0.111 [-0.055-0.277] negligible
#  6 MDS-UPDRS III Tremor     Month 12    152       0.5        2.15 0.232  0.082        0.071        0.393 small         0.232 [0.071-0.393] small      
#  7 Non-Tremor Scale         Month 6     140       1.52       6.50 0.234  0.086        0.066        0.402 small         0.234 [0.066-0.402] small      
#  8 Non-Tremor Scale         Month 12    152       2.29       7.41 0.309  0.083        0.146        0.472 small         0.309 [0.146-0.472] small      
#  9 Tremor Scale             Month 6     140       0.343      2.10 0.163  0.085       -0.004        0.33  negligible    0.163 [-0.004-0.33] negligible 
# 10 Tremor Scale             Month 12    152       0.638      2.35 0.271  0.083        0.109        0.433 small         0.271 [0.109-0.433] small      
# 11 11-Item Scale            Month 6     140       0.557      3.02 0.184  0.085        0.017        0.351 negligible    0.184 [0.017-0.351] negligible 
# 12 11-Item Scale            Month 12    152       1.34       3.39 0.394  0.084        0.229        0.559 small         0.394 [0.229-0.559] small     
...
# 1 MDS-UPDRS Part III       Month 6     140       1.2        5.85 0.205 [0.038-0.373] small      
#  2 MDS-UPDRS Part III       Month 12    152       1.48       7.06 0.21 [0.049-0.37] small        
#  3 MDS-UPDRS III Non-Tremor Month 6     140       0.986      5.27 0.187 [0.02-0.354] negligible  
#  4 MDS-UPDRS III Non-Tremor Month 12    152       0.98       5.96 0.165 [0.005-0.325] negligible 
#  5 MDS-UPDRS III Tremor     Month 6     140       0.214      1.93 0.111 [-0.055-0.277] negligible
#  6 MDS-UPDRS III Tremor     Month 12    152       0.5        2.15 0.232 [0.071-0.393] small      
#  7 Non-Tremor Scale         Month 6     140       1.52       6.50 0.234 [0.066-0.402] small      
#  8 Non-Tremor Scale         Month 12    152       2.29       7.41 0.309 [0.146-0.472] small      
#  9 Tremor Scale             Month 6     140       0.343      2.10 0.163 [-0.004-0.33] negligible 
# 10 Tremor Scale             Month 12    152       0.638      2.35 0.271 [0.109-0.433] small      
# 11 11-Item Scale            Month 6     140       0.557      3.02 0.184 [0.017-0.351] negligible 
# 12 11-Item Scale            Month 12    152       1.34       3.39 0.394 [0.229-0.559] small   
...
ggsave("../out/smd_scale_comparison.svg", smd_overall_plot, width = 7, height = 5)
...
# 4 MDS-UPDRS III Tremor       0.232     4 small        
# 5 MDS-UPDRS Part III         0.21      5 small        
# 6 MDS-UPDRS III Non-Tremor   0.165     6 negligible  
...
scales_to_test <- c("MDS.UPDRS.Part.III", "MDS.UPDRS.III.Non.Tremor", "MDS.UPDRS.III.Tremor", "Non.Tremor.Scale", "Tremor.Scale", "X11.Item.Scale")
...
forest_data <- univariate_results %>%
  mutate(
    scale_clean = case_when(
      ...
      scale == "MDS.UPDRS.III.Non.Tremor" ~ "MDS-UPDRS III Non-Tremor",
      scale == "MDS.UPDRS.III.Tremor" ~ "MDS-UPDRS III Tremor",
    ...
    scale_clean = factor(scale_clean, 
                         levels = c("MDS-UPDRS Part III",  "MDS-UPDRS III Non-Tremor",
                                    "MDS-UPDRS III Tremor",
                                    "Non-Tremor Scale", 
                                    "11-Item Scale", 
                                    "Tremor Scale")),
...
ggsave("../out/ledd_scales_forest_with_hr.svg", hr_plot_with_hr, width = 10, height = 4)
